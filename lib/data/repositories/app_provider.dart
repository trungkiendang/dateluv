...
class AppProvider extends ChangeNotifier {
  ...
  bool get isSyncEnabled => _currentCoupleId != null;
  ...
  Future<void> signOut() async {
    ...
    // Stop sync and clear subscriptions
    _profileSub?.cancel();
    _diarySub?.cancel();
  ...
  void startSync(String coupleId) {
    ...
    _pushLocalDataToCloud(coupleId);
  ...
  void _pushLocalDataToCloud(String coupleId) async {
    // Sync profile
    if (_coupleProfile != null) {
      await _syncService.syncCoupleProfile(coupleId, _coupleProfile!);
    }

    // Sync diary entries
    for (var entry in _diaryEntries) {
      await _syncService.syncDiaryEntry(coupleId, entry);
    }

    // Sync milestones (non-auto)
    for (var milestone in _milestones) {
      if (!milestone.isAutoGenerated) {
        await _syncService.syncMilestone(coupleId, milestone);
      }
    }
  }
